...
mod test {
    use super::*;
    use elliptic_curve::array::Array;
    use elliptic_curve::{PrimeField, consts::U28};
    use hash2curve::{ExpandMsgXof, hash_to_field};
    use hex_literal::hex;
    use sha3::Shake256;
    ...
    fn scalar_hash() {
        ...
        let [res] =
            hash_to_field::<1, DecafScalar, ExpandMsgXof<Shake256>, U28>(&[msg], &[dst]).unwrap();
    ...
    fn hash_to_scalar_voprf() {
        ...
        'outer: for test_vector in TEST_VECTORS {
            for counter in 0_u8..=u8::MAX {
                let [scalar] = hash_to_field::<1, DecafScalar, ExpandMsgXof<Shake256>, U28>(
